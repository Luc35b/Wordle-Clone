///TODO: 1. Python script to remove all words that end with a single 's'.
///      2. Add jump animation to correct word guess. 
///      3. Replicate Navbar, startup popup, win/lose screen w/ progress bar, and change font; Fix changing screen sizes.
///      4. Create accounts w/ login and password that track score.
///      5. Create tokens/sessions that'll save current progress and if daily attempt was made.

///TO RUN with import: 'http-server' in terminal
...
const WORDS = [
    ...
    "SADLY", "SAFER", "SAINT", "SALAD", "SALLY", "SALON", "SALSA", "SALTY", "SALVE", "SALVO", "SANDY", "SANER", "SAPPY", "SASSY", "SATIN", "SATYR", "SAUCE", "SAUCY", "SAUNA", "SAUTE", "SAVOR", "SAVOY", "SAVVY", "SCALD", "SCALE", "SCALP", "SCALY", "SCAMP", "SCANT", "SCARE", "SCARF", "SCARY", "SCENE", "SCENT", "SCION", "SCOFF", "SCOLD", "SCONE", "SCOOP", "SCOPE", "SCORE", "SCORN", "SCOUR", "SCOUT", "SCOWL", "SCRAM", "SCRAP", "SCREE", "SCREW", "SCRUB", "SCRUM", "SCUBA", "SEDAN", "SEEDY", "SEGUE", "SEIZE", "SEMEN", "SENSE", "SEPIA", "SERIF", "SERUM", "SERVE", "SETUP", "SEVEN", "SEVER", "SEWER", "SHACK", "SHADE", "SHADY", "SHAFT", "SHAKE", "SHAKY", "SHALE", "SHALL", "SHALT", "SHAME", "SHANK", "SHAPE", "SHARD", "SHARE", "SHARK", "SHARP", "SHAVE", "SHAWL", "SHEAR", "SHEEN", "SHEEP", "SHEER", "SHEET", "SHEIK", "SHELF", "SHELL", "SHIED", "SHIFT", "SHINE", "SHINY", "SHIRE", "SHIRK", "SHIRT", "SHOAL",
    'SHOCK', 'SHONE', 'SHOOK', 'SHOOT', 'SHORE', 'SHORN', 'SHORT', 'SHOUT', 'SHOVE', 'SHOWN', 'SHOWY',
    'SHREW', 'SHRUB', 'SHRUG', 'SHUCK', 'SHUNT', 'SHUSH', 'SHYLY', 'SIEGE', 'SIEVE', 'SIGHT', 'SIGMA',
    'SILKY', 'SILLY', 'SINCE', 'SINEW', 'SINGE', 'SIREN', 'SISSY', 'SIXTH', 'SIXTY', 'SKATE', 'SKIER',
    'SKIFF', 'SKILL', 'SKIMP', 'SKIRT', 'SKULK', 'SKULL', 'SKUNK', 'SLACK', 'SLAIN', 'SLANG', 'SLANT',
    'SLASH', 'SLATE', 'SLEEK', 'SLEEP', 'SLEET', 'SLEPT', 'SLICE', 'SLICK', 'SLIDE', 'SLIME', 'SLIMY',
    'SLING', 'SLINK', 'SLOOP', 'SLOPE', 'SLOSH', 'SLOTH', 'SLUMP', 'SLUNG', 'SLUNK', 'SLURP', 'SLUSH',
    'SLYLY', 'SMACK', 'SMALL', 'SMART', 'SMASH', 'SMEAR', 'SMELL', 'SMELT', 'SMILE', 'SMIRK', 'SMITE',
    'SMITH', 'SMOCK', 'SMOKE', 'SMOKY', 'SMOTE', 'SNACK', 'SNAIL', 'SNAKE', 'SNAKY', 'SNARE', 'SNARL',
    'SNEAK', 'SNEER', 'SNIDE', 'SNIFF', 'SNIPE', 'SNOOP', 'SNORE', 'SNORT', 'SNOUT', 'SNOWY', 'SNUCK',
    'SNUFF', 'SOAPY', 'SOBER', 'SOGGY', 'SOLAR', 'SOLID', 'SOLVE', 'SONAR', 'SONIC', 'SOOTH', 'SOOTY',
    'SORRY', 'SOUND', 'SOUTH', 'SOWER', 'SPACE', 'SPADE', 'SPANK', 'SPARE', 'SPARK', 'SPASM', 'SPAWN',
    'SPEAK', 'SPEAR', 'SPECK', 'SPEED', 'SPELL', 'SPELT', 'SPEND', 'SPENT', 'SPERM', 'SPICE', 'SPICY',
    'SPIED', 'SPIEL', 'SPIKE', 'SPIKY', 'SPILL', 'SPILT', 'SPINE', 'SPINY', 'SPIRE', 'SPITE', 'SPLAT',
    'SPLIT', 'SPOIL', 'SPOKE', 'SPOOF', 'SPOOK', 'SPOOL', 'SPOON', 'SPORE', 'SPORT', 'SPOUT', 'SPRAY',
    'SPREE', 'SPRIG', 'SPUNK', 'SPURN', 'SPURT', 'SQUAD', 'SQUAT', 'SQUIB', 'STACK', 'STAFF', 'STAGE',
    'STAID', 'STAIN', 'STAIR', 'STAKE', 'STALE', 'STALK', 'STALL', 'STAMP', 'STAND', 'STANK', 'STARE',
    'STARK', 'START', 'STASH', 'STATE', 'STAVE', 'STEAD', 'STEAK', 'STEAL', 'STEAM', 'STEED', 'STEEL',
    'STEEP', 'STEER', 'STEIN', 'STERN', 'STICK', 'STIFF', 'STILL', 'STILT', 'STING', 'STINK', 'STINT',
    'STOCK', 'STOIC', 'STOKE', 'STOLE', 'STOMP', 'STONE', 'STONY', 'STOOD', 'STOOL', 'STOOP', 'STORE',
    'STORK', 'STORM', 'STORY', 'STOUT', 'STOVE', 'STRAP', 'STRAW', 'STRAY', 'STRIP', 'STRUT', 'STUCK',
    'STUDY', 'STUFF', 'STUMP', 'STUNG', 'STUNK', 'STUNT', 'STYLE', 'SUAVE', 'SUGAR', 'SUING', 'SUITE',
    'SULKY', 'SULLY', 'SUMAC', 'SUNNY', 'SUPER', 'SURER', 'SURGE', 'SURLY', 'SUSHI', 'SWAMI', 'SWAMP',
    'SWARM', 'SWASH', 'SWATH', 'SWEAR', 'SWEAT', 'SWEEP', 'SWEET', 'SWELL', 'SWEPT', 'SWIFT', 'SWILL',
    'SWINE', 'SWING', 'SWIRL', 'SWISH', 'SWOON', 'SWOOP', 'SWORD', 'SWORE', 'SWORN', 'SWUNG', 'SYNOD','SYRUP',
...
const winMes = ['Genious','Magnificent','Impressive','Splendid','Great','Phew']; 
...
function initialize() {
    ...
    document.addEventListener("keyup", (e) => { // Physcal Keyboard
        if (gameOver || isAnimating) {
            return;
            //Add restart button function - location.reload();
        }
        ...
        if ("KeyA" <= e.code && e.code <= "KeyZ") {
            if (column < width) {
                ...
                if (currentTile.innerText == "") {
                    ...
                    column++;
                    enteredWord = enteredWord.concat(currentTile.innerText);
                }
            }
        }
...
function setupKeyboard() { // Digital Keyboard
    ...
    keys.forEach(key => {
        key.addEventListener("click", (e) => {
            ...
            if (button.id === "backspace") {
                ...
                const currentTile = document.getElementById(row.toString() + "-" + column.toString());
                if (currentTile) currentTile.innerText = "";
                currentTile.classList.remove("filled");
                currentTile.classList.remove("pop");
                enteredWord = enteredWord.slice(0, -1); 
            } else if (keyText === "ENTER" && column == 5) {
                if (!isValidWord(enteredWord)) {
                    shakeRow();
                } else {
                    update();
            ...
            } else if ("QWERTYUIOPASDFGHJKLLZXCVBNM".includes(keyText) && keyText !== "") {
                if (column < width) {
                    const currentTile = document.getElementById(row.toString() + "-" + column.toString());
                    currentTile.classList.add("filled");
                    currentTile.classList.add("pop");
                    if (currentTile && currentTile.innerText === "") {
                        currentTile.innerText = keyText;
                        column++;
                        enteredWord = enteredWord.concat(keyText);
                    }
                }
            }
        });
...
function isValidWord(check) {
    return WORDS.includes(check.toUpperCase());
}
...
function shakeRow() {
    ...
    const tiles = Array.from(document.querySelectorAll(`#board .tile`)).slice(startIndex, endIndex);
    ...
    tiles[0].addEventListener('animationend', () => {
        tiles.forEach(tile => {
...
function animateTiles(statusArray, keyStatus) {
    const startIndex = row * width; 
    const endIndex = startIndex + width;
...
function update() {
    ...
    for (let i = 0; i < width; i++) {
        ...
        if (chosenWord[i] === letter) {
            statusArray.push(1); // Correct
            correct++;
            tempWord = tempWord.slice(0, i) + '-' + tempWord.slice(i + 1);
            yellow.push("");
            doJump++;
            if (yellow.includes(letter) && !tempWord.includes(letter)) {
                let remove = yellow.indexOf(letter);
                statusArray[remove] = 3;
                yellow[remove] = "";
            } else if (yellow.includes(letter) && tempWord.includes(letter)) {
                let remove = yellow.indexOf(letter);
                tempWord = tempWord.slice(0, tempWord.indexOf(letter)) + '-' + tempWord.slice(tempWord.indexOf(letter)+1);
                keyStatus[remove] = 'present';
            }
            keyStatus[letter] = 'correct';
        } else if (tempWord.includes(letter)) {
            tempWord = tempWord.slice(0, tempWord.indexOf(letter)) + '-' + tempWord.slice(tempWord.indexOf(letter)+1);
            statusArray.push(2); // Present
            yellow.push(letter);
            if (keyStatus[letter] !== 'correct') {
                keyStatus[letter] = 'present';
            }
        } else {
            statusArray.push(3); // Absent
            yellow.push("");
            if (!keyStatus[letter]) {
    ...
    animateTiles(statusArray, keyStatus);

    if (correct === width) {
        setTimeout(() => { 
            gameOver = true;
            showPopup(winMes[row]);
        }, 1900);
    }else if(row === 6){
        setTimeout(() => { 
            gameOver = true;
            showPopup(chosenWord);
        }, 1900);
    }
    
}

function updateKeys(keyStatus) {
    for (const [letter, status] of Object.entries(keyStatus)) {
        const keyElement = Array.from(document.querySelectorAll(".key")).find(el => el.textContent === letter);
        if (keyElement) {
